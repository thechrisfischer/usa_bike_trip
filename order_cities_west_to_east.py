#!/usr/bin/env python3
"""
Order cities from west to east based on longitude
"""
...
def main():
    ...
    # These are rough longitude estimates for major cities
    city_longitudes = {
    ...
    print("=== CITIES IN ROUTE ORDER (WEST TO EAST) ===")
    print()
    ...
    with open('cities_west_to_east.csv', 'w', newline='') as f:
        writer = csv.writer(f)
        writer.writerow(['Order', 'City', 'Longitude'])
        for i, (city, longitude) in enumerate(cities_with_longitude, 1):
            writer.writerow([i, city, longitude])
    
    with open('cities_west_to_east.txt', 'w') as f:
        f.write("Cities visited during cross-country bike trip (west to east):\n\n")
        for i, (city, longitude) in enumerate(cities_with_longitude, 1):
            f.write(f"{i:2d}. {city}\n")
    
    print(f"\nResults saved to:")
    print(f"  - cities_west_to_east.csv")
    print(f"  - cities_west_to_east.txt")
